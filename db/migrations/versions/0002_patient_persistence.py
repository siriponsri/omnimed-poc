...
revision = "0002_patient_persistence"
...
def upgrade():
    op.create_table(
        ...
        *audit(),
        sa.ForeignKeyConstraint(["tenant_id"], ["foundation_tenant.id"]),
    )
    op.create_table(
        "party",
    ...
    op.create_table(
        ...
        sa.Column("tenant_id", sa.Uuid(), nullable=False),
        sa.Column("party_id", sa.Uuid(), nullable=False),
        sa.Column("hn", sa.String(32), nullable=False),
...
def downgrade():
    op.drop_index("uq_patient_identifier_preferred", table_name="patient_identifier")
    op.drop_table("patient_identifier")
    op.drop_table("patient")
    op.drop_table("party_relation")
    op.drop_table("party")
    op.drop_table("patient_hn_counter")
